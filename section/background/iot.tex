...
%	\item Can not use a normal ISO model due to high overhead.
	\item SHORT about zigbee, bluetooth.
\end{itemize}

IoT networks is designed after the traditional OSI model, where devices uses the different protocol layers, shown in figure \ref{fig:ip-6lowpan-stack}, in order to communicate with each other. Due to the devices performance constraint and power limitations, some regular protocols in the communciation layers are to heavy to be used for in the IoT domain. 
